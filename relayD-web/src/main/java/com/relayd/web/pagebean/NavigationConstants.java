package com.relayd.web.pagebean;

/**
 * Bloch, Joshua, Effective Java, 2nd Edition, p. 98
 *
 * @author schmollc (Christian@relayd.de)
 * @since 16.08.2016
 *
 */
public final class NavigationConstants {

	private NavigationConstants() {
		// restrict instantiation
	}

	public static final String RELAY_EDIT_DIALOG_ID = "relayEditDialog";

	public static final String RELAY_BROWSE_ADD_PARTICIPANT_DIALOG_ID = "relayBrowseAddParticipantDialog";

	public static final String MEMBER_EDIT_DIALOG_ID = "memberEditDialog";

	public static final String PARTICIPANT_EDIT_DIALOG_ID = "participantEditDialog";

	public static final String PERSON_PERSON_DIALOG_ID = "personEditDialog";

	public static final String RELAY_EVENT_EDIT_DIALOG_ID = "event/relayEventEditDialog";

	public static final String RELAY_EVENT_EDIT_ADD_PERSON_DIALOG_ID = "relayEventEditAddPersonDialog";
}